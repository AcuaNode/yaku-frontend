    ...
    <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 1px 6px rgba(0,0,0,0.07)', padding: '20px', display: 'flex', flexDirection: 'column', gap: '0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
          <span style={{ color: '#64748b' }}>Código Físico:</span>
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: '13px', color: '#94a3b8', textAlign: 'center', lineHeight: 1.5, fontWeight: 500 }}>
        Añadir nuevo dispositivo<br />de monitoreo u operación
      </span>
    </button>
  )
}
...
export default function EquiposPage() {
  ...
  const statItems = [
    { label: 'Total Equipos',        value: stats?.totalEquipos ?? 0,         color: '#0f172a' },
    { label: 'Sensores Activos',      value: stats?.sensoresActivos ?? 0,       color: '#0d9488' },
    { label: 'Bombas en Operación',   value: stats?.bombasEnOperacion ?? 0,     color: '#0f172a' },
    { label: 'Requiere Mantención',   value: stats?.requiereMantension ?? 0,    color: '#ef4444' },
  ]
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: '26px', fontWeight: 800, color: '#0f172a', margin: '0 0 4px' }}>Gestión de Equipos</h1>
          <p style={{ fontSize: '14px', color: '#64748b', margin: 0 }}>Monitorea y configura el hardware desplegado en las instalaciones.</p>
        </div>
        <button
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '16px' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '12px', width: '100%', maxWidth: '460px', boxShadow: '0 20px 60px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 28px 20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#0f172a', margin: 0 }}>Registrar Equipo</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', padding: '0 28px 28px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Nombre del equipo</label>
                <input
                  type="text"
                  placeholder="Ej: Sensor Oxígeno Pro-V2"
                  value={form.nombre}
              ...
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Tipo de equipo</label>
                <div style={{ display: 'flex', gap: '10px' }}>
                  {(['SENSOR', 'PUMP'] as TipoEquipo[]).map(t => (
                    <button
                      key={t}
                      type="button"
                      ...
                      style={{
                        ...
                        backgroundColor: form.tipo === t ? TIPO_BADGE[t].bg : 'transparent',
                        color: form.tipo === t ? TIPO_BADGE[t].color : '#64748b',
                        border: form.tipo === t ? `2px solid ${TIPO_BADGE[t].bg}` : '2px solid #e2e8f0',
                      }}
                    >
                      {t}
                    </button>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Código Físico</label>
                <input
                  type="text"
                  placeholder="Ej: LOC-A2-S4"
              ...
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Ubicación</label>
                <input
                  type="text"
                  placeholder="Ej: Estanque Tilapia 04"
